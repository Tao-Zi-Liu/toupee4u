  ...
  async () => {
    ...
    try {
      ...
      if (!Array.isArray(insights) || insights.length === 0) {
        console.error("Invalid insights response"); return;
      }
      const today = new Date().toISOString().split('T')[0];
      await db.collection('dailyInsights').doc(today).set({
        date: today, insights, generatedAt: admin.firestore.FieldValue.serverTimestamp(),
      });
...
export const generatePersonalizedInsight = onCall(
  { secrets: ["GEMINI_API_KEY"], memory: "256MiB", timeoutSeconds: 30, region: "us-central1" },
  async (request) => {
...
export const generateInsightsManual = onCall(
  { secrets: ["GEMINI_API_KEY"], memory: "256MiB", timeoutSeconds: 60, region: "us-central1" },
  async (request) => {
