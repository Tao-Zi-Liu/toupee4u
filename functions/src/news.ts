...
const NEWS_PROMPT = (topic: string, isChinese: boolean = false) => `
...
function selectTopics(pool: string[], count: number, seed: number): string[] {
  const shuffled = [...pool];
...
async function generateNewsArticles(ai: GoogleGenAI, maxArticles: number = 15): Promise<any[]> {
  ...
  const allTopics: Array<{ topic: string; isChinese: boolean }> = [
  ...
  async function searchTopic(topic: string, isChinese: boolean): Promise<any[]> {
    try {
      ...
      if (isChinese) valid.forEach(a => { a.sourceUrl = ''; });
...
export const generateDailyNews = onSchedule(
  ...
  async () => {
    ...
    if (!apiKey) { console.error("GEMINI_API_KEY secret not found"); return; }
...
export const generateNewsManual = onCall(
  { secrets: ["GEMINI_API_KEY"], memory: "512MiB", timeoutSeconds: 540, region: "us-central1" },
  async (request) => {
